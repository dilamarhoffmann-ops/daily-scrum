...
import { 
  ...
  Clock, User, Plus, Trash2, Flag 
} from 'lucide-react';
import useAgileStore from '../../store/useAgileStore';

export default function DailyMeeting() {
  const { tasks, userStories, daily_items, addDailyItem, deleteDailyItem, activeProjectId, updateTask, projects } = useAgileStore();
  ...
  return (
    <div className="flex flex-col gap-10">
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-100 transition-all duration-500">
        ...
        <div className="flex items-center gap-6 bg-white dark:bg-slate-900 px-4 py-2.5 rounded-xl border border-slate-100 dark:border-slate-800 shadow-xl self-start transition-colors">
           ...
           <div className="flex gap-2">
             <button 
               ...
               className={`p-2.5 rounded-xl transition-all ${isProjectPaused ? 'opacity-50 cursor-not-allowed text-slate-400 bg-slate-100 dark:bg-slate-800' : isActive ? 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400' : 'bg-brand-primary text-white shadow-lg shadow-brand-primary/25'}`}
             >
             ...
             <button 
               ...
               className="p-2.5 bg-slate-50 dark:bg-slate-950 text-slate-400 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
             >
               <RotateCcw size={16} />
             </button>
           </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  {userTasks.map(t => {
                    ...
                    const story = t.story_id 
                      ? sortedStories.find(s => s.id === t.story_id)
                      : null;
                    const storyPriority = t.story_id 
                      ? sortedStories.findIndex(s => s.id === t.story_id) + 1
                      : 0;
                        ...
                        `}>
                          <div className={`badge-harmony absolute top-4 right-4 ${t.is_blocked ? 'badge-harmony-danger' : 'badge-harmony-neutral'} border-2 flex items-center gap-1`}>
                            ...
                            <input 
                               ...
                               type="number"
                               min="0"
                               className="w-8 bg-transparent text-right outline-none text-[10px] font-black p-0 m-0 disabled:opacity-60 disabled:cursor-not-allowed"
                          ...
                          <div className="flex-1 pr-6 pt-2">
                            {/* Story Context */}
                             {story && (
                               <div className="mb-2 flex items-center justify-between gap-1.5 opacity-70">
            ...
            {Object.keys(tasksByUser).length === 0 && (
              <div className="md:col-span-2 p-5 border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-2xl text-center">
                <p className="text-slate-400 dark:text-slate-600 text-xs font-bold uppercase tracking-widest">Nenhuma tarefa ativa neste projeto</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-8">
           ...
           <div className="glass-card p-6 flex flex-col gap-6 transition-colors border border-slate-100">
             ...
             <form onSubmit={handleAddItem} className="flex flex-col gap-4">
               <fieldset disabled={isProjectPaused} className="flex flex-col gap-4 w-full p-0 m-0 border-0 disabled:opacity-60">
                 ...
                 <div className="flex flex-col gap-2">
                   ...
                   <select 
                     ...
                     className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-xl px-4 py-3 text-xs font-bold text-slate-800 dark:text-slate-200 outline-none focus:border-brand-primary appearance-none cursor-pointer disabled:cursor-not-allowed"
                   >
                 ...
                 <textarea 
                   ...
                   className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-xl p-4 text-xs font-bold text-slate-800 dark:text-slate-200 outline-none focus:border-brand-primary min-h-[80px] placeholder:text-slate-400 dark:placeholder:text-slate-700 disabled:cursor-not-allowed"
                 />
                 <button type="submit" className="w-full bg-slate-900 dark:bg-slate-100 dark:text-slate-900 text-white py-2 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-black dark:hover:bg-white transition-all shadow-lg shadow-black/10 disabled:cursor-not-allowed">
                   Adicionar Card
                 </button>
               </fieldset>
             </form>
             ...
             <div className="flex flex-col gap-4">
                 {filteredDailyItems.map(item => (
                   <div key={item.id} className={`p-4 rounded-xl border-2 transition-all group relative overflow-hidden ${item.type === 'action' ? 'bg-blue-50/50 border-blue-100 dark:bg-blue-900/10 dark:border-blue-900/30' : 'bg-[#fff0f0]/50 border-[#8B0000]/20 dark:bg-[#8B0000]/10 dark:border-[#8B0000]/30'}`}>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <p className="text-lg font-black text-slate-800 dark:text-slate-200 leading-snug tracking-tight">{item.content}</p>
                    </div>
                ...
                {filteredDailyItems.length === 0 && (
                  <div className="w-full p-5 border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-2xl flex flex-col items-center justify-center gap-3">
                    <MessageSquare size={18} className="text-slate-200 dark:text-slate-800" />
